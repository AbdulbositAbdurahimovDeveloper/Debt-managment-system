pipeline {
    ...
    stages {
        ...
        stage('3. Deploy Application') {
            steps {
                echo "Eski konteyner o'chirilmoqda..."
                sh "docker rm -f ${CONTAINER_NAME} || true"

                // Faqat ikkita faylni chaqiramiz: .env va google-key.json
                withCredentials([
                    ...
                    echo "Konteyner --env-file yordamida ishga tushirilmoqda..."
                    sh """
                        docker run -d \
                          --name "${CONTAINER_NAME}" \
                          -p ${APP_PORT}:8080 \
                ...
                echo "Ilova muvaffaqiyatli ishga tushirildi: Port ${APP_PORT}"
            }
        }
